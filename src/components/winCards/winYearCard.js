...
const datesAreOnSameYear = (first, second) =>
...
const calculateAllPositives = (player, stats) => {
    ...
    if (stats) {
        stats.map(x => {
            ...
            if (x.stat.isPositive && x.stat.playerName === player && datesAreOnSameYear(statDay, new Date())) {
                positiveStats++;
            }
        })
    }
...
const calculateAllNegatives = (player, stats) => {
    ...
    if (stats) {
        stats.map(x => {
            let statDay = new Date(x.stat.createdAt.seconds * 1000);
            if (!x.stat.isPositive && x.stat.playerName === player && !neutrals.includes(x.stat.statName) && datesAreOnSameYear(statDay, new Date())) {
                negativeStats++;
            }
        })
    }
