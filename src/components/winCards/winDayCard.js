...
const calculateAllPositives = (player, stats) => {
    ...
    if (stats) {
        stats.map(x => {
            ...
            if (x.stat.isPositive && x.stat.playerName === player && datesAreOnSameDay(statDay, new Date())) {
                positiveStats++;
            }
        })
    }
...
const calculateAllNegatives = (player, stats) => {
    ...
    if (stats) {
        stats.map(x => {
            let statDay = new Date(x.stat.createdAt.seconds * 1000);
            if (!x.stat.isPositive && x.stat.playerName === player && !neutrals.includes(x.stat.statName) && datesAreOnSameDay(statDay, new Date())) {
                negativeStats++;
            }
        })
    }
...
const WinDayCard = ({ players, stats }) => {
    let playerArray = overallRanking(players, stats);
    if (playerArray.length > 1) {
        return (
            <div className="bg-white shadow p-4 rounded mb-4">
                <h1 className="font-bold border-b mb-2 pb-2">Win The <span className="text-red-600">Day</span></h1>
...
const ACard = connect(mapStateToProps)(WinDayCard);
